...
def download(label3):
    ...
    response = requests.put(f"http://{constants.ip}:{constants.port}/download/", data=filename)
...
def upload(label3):
    ...
    response = requests.put(f"http://{constants.ip}:{constants.port}/upload/", data=message)
